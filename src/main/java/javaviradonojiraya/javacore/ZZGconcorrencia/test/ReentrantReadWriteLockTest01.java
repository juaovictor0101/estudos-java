...
class MapReadWritter {
    private final Map<String, String> map = new LinkedHashMap<>();
    private final ReentrantReadWriteLock rwl;

    MapReadWritter(ReentrantReadWriteLock rwl) {
        this.rwl = rwl;
    }
...
public class ReentrantReadWriteLockTest01 {
    public static void main(String[] args) {
        ...
        MapReadWritter mapReadWritter = new MapReadWritter(rwl);

        Runnable writer = () -> {
            for (int i = 0; i < 20; i++) {
                mapReadWritter.put(String.valueOf(i), String.valueOf(i));
            }
        };
        Runnable reader = () -> {
            if (rwl.isWriteLocked()) {
            ...
            try {
                System.out.println(Thread.currentThread().getName() + " " + mapReadWritter.allKeys());
            } finally {
                rwl.readLock().unlock();
            }
        };
